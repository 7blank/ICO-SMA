...
class rs_agent(Agent):
    ...
    def algo_RS(C,M_time,n=100_000, weight_K=10_000, weight_q=10_000, weight_t=3, weight_d=13, weight_c=10):
        M = cost_matrix(C)
        # starting with solution from the pool ? 
        # add in the arguments of algo_RS "start_solution = random_solution(C)" and change R to "R = start_solution"
        R = random_solution(C)   
    ...
    def step(self):
        self.solution = closing_tour(algo_RS(self.model.clients)[0])
